...
<div :style='{"width":"100%","padding":"20px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}'  label="Singer" prop="geshou">
            <el-select v-model="ruleForm.geshou" placeholder="Please selectSinger"  >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="play back" prop="bofang">
            <file-upload
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('yinlebangdan/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('yinlebangdan/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('yinlebangdan/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
      ...
      bofangUploadChange(fileUrls) {
          this.ruleForm.bofang = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
