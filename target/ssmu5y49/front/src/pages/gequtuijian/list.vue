...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7% 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>Song name</div>
        <el-input v-model="formSearch.songname" placeholder="Song name" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>Music classification</div>
        <el-select v-model="formSearch.yinlefenlei" placeholder="Please selectMusic classification" :clearable="true">
          <el-option v-for="(item, index) in yinlefenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>Singer</div>
        <el-input v-model="formSearch.singer" placeholder="Singer" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>album</div>
        <el-input v-model="formSearch.zhuanji" placeholder="album" clearable></el-input>
      </el-form-item>
	  <el-button v-if=" true " :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"0","background":"#000","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="getList(1, curFenlei)"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-search"></i>inquire</el-button>
	  <el-button v-if="isAuth('gequxinxi','new')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"0","background":"#999999","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/gequxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"padding":"20px 7% 20px","margin":"20px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				...
				<img :style='{"width":"100%","boxShadow":"0px 2px 6px #ccc","margin":"0 0 8px","objectFit":"cover","display":"block","height":"260px"}' v-if="item.fengmian && item.fengmian.substr(0,4)=='http'" :src="item.fengmian" class="image" />
				<img :style='{"width":"100%","boxShadow":"0px 2px 6px #ccc","margin":"0 0 8px","objectFit":"cover","display":"block","height":"260px"}' v-else :src="baseUrl + (item.fengmian?item.fengmian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"4px 0px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","width":"100%","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis"}' class="name ">{{item.songname}}</div>
				<div :style='{"padding":"4px 0px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","width":"100%","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis"}' class="name ">{{item.yinlefenlei}}</div>
				<div :style='{"padding":"4px 0px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","width":"100%","lineHeight":"24px","fontSize":"16px","textOverflow":"ellipsis"}' class="name ">{{item.singer}}</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.curFenlei != 'All') searchWhere.yinlefenlei = this.curFenlei;
        this.$http.get('gequxinxi/tuijian', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/gequxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    playClick(item) {
