	...
	<div class="addEdit-block" :style='{"padding":"30px 0 0 0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="Musical style" prop="yinlefengge">
					<el-input v-model="ruleForm.yinlefengge" placeholder="Musical style" clearable  :readonly="ro.yinlefengge"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="Musical style" prop="yinlefengge">
					<el-input v-model="ruleForm.yinlefengge" placeholder="Musical style" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="Issue time" prop="faxingshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.faxingshijian" label="Issue time" prop="faxingshijian">
					<el-input v-model="ruleForm.faxingshijian" placeholder="Issue time" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-if="type!='info'"  label="Album introduction" prop="zhuanjijieshao">
					<editor 
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.zhuanjijieshao" label="Album introduction" prop="zhuanjijieshao">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.zhuanjijieshao"></span>
                </el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.faxingshijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='yinlefengge'){
							this.ruleForm.yinlefengge = obj[o];
							this.ro.yinlefengge = true;
							continue;
						}
						if(o=='faxingshijian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.zhuanjijieshao = this.ruleForm.zhuanjijieshao.replace(reg,'../../../ssmu5y49/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yinlezhuanji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yinlezhuanjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
